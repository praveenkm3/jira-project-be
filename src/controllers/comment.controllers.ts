...
import { addCommentService, deleteCommentService, editCommentService } from "../services/comment_services.ts";
...
export const editComment =async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const result=await editCommentService(commentId as string,userId,comment);
    return res.status(200).json(result);
  } catch (error) {
    next(error);
  }
